...
const SupplierMaster = () => {
  ...
  const [formData, setFormData] = useState({ name: '', gstin: '', pan: '', address: '', city: '', state: 'Gujarat', state_code: '24', pincode: '', phone: '', email: '' });

  useEffect(() => { fetchSuppliers(); }, []);

  const fetchSuppliers = async () => {
    try { const response = await api.get('/suppliers'); setSuppliers(response.data); } 
  ...
  const closeModal = () => { setShowModal(false); setEditingSupplier(null); setFormData({ name: '', gstin: '', pan: '', address: '', city: '', state: 'Gujarat', state_code: '24', pincode: '', phone: '', email: '' }); };

  const filtered = suppliers.filter(s => !searchTerm || s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.gstin?.includes(searchTerm.toUpperCase()));

  if (loading) return <div className="loading-container"><div className="spinner"></div></div>;

  return (
    <div data-testid="supplier-master">
      <div className="page-header"><div><h1>Supplier Master</h1></div><button className="btn btn-primary" onClick={() => setShowModal(true)}><Plus size={16} /> Add Supplier</button></div>
      ...
      <div className="card">
        <table className="data-grid">
          ...
          <tbody>{filtered.map(s => (<tr key={s.id}><td><strong>{s.name}</strong></td><td>{s.gstin}</td><td>{s.city}</td><td>{s.phone}</td><td className="text-center"><button className="btn btn-sm btn-secondary" onClick={() => handleEdit(s)}><Edit2 size={14} /></button></td></tr>))}</tbody>
        </table>
      </div>
      {showModal && (
        <div className="modal-overlay"><div className="modal" style={{width: '600px'}}>
          <div className="modal-header"><h3>{editingSupplier ? 'Edit' : 'Add'} Supplier</h3><button className="modal-close" onClick={closeModal}>&times;</button></div>
